...
void CManagePSWDPols::OnEdit()
{
  ...
  if (rc == IDOK) {
    ...
    // Save changes for Undo/Redo
    st_PSWDPolicyChange st_change;
    st_change.name = m_iSelectedItem != 0 ? cs_policyname.GetString() : L"";
...
void CManagePSWDPols::OnPolicyRightClick(NMHDR * /*pNotifyStruct*/, LRESULT *pLResult)
{
  *pLResult = 0; // Perform default processing on return
  ...
  const StringX sxPolicyName = m_PolicyNames.GetItemText(nItem, 0).GetString();
...
void CManagePSWDPols::OnEntryDoubleClicked(NMHDR *, LRESULT *pLResult)
{
  ...
  *pLResult = 1L;
  ...
  // Get entry's details
  StringX sxGroup = m_PolicyEntries.GetItemText(nIndex, 0).GetString();
  StringX sxTitle = m_PolicyEntries.GetItemText(nIndex, 1).GetString();
  StringX sxUser  = m_PolicyEntries.GetItemText(nIndex, 2).GetString();
