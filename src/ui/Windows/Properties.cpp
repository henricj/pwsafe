...
BOOL CProperties::OnInitDialog()
{
  ...
  CString csDBName = m_pdbp->db_name.empty() ? cs_text : CString(m_pdbp->db_name.c_str());
  if (csDBName.GetLength() > 32)
    csDBName = csDBName.Left(30) + L"...";

  CString csDBDescription = m_pdbp->db_description.empty() ? cs_text : CString(m_pdbp->db_description.c_str());
